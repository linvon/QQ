...
void FriendButton::clickedFriendButton()
{
    ...
    if(mesVec.size()>0)
    {
        for(qint32 i=0;i<mesVec.size();i++)
            getRoomPoint()->setOutputLine(mesVec[i]);
        mesVec.clear();
        haveMessage=false;
        count::cnt--;
        setButtonStatus();
...
void FriendButton::setButtonStatus()
{
	...
	if(friInf.remark.isEmpty())
		...
		mark = friInf.remark;
	//如果对方是离线或隐身，就将他们的的头像设为黑白，并且不显示状态
...
void FriendButton::changeStatus(int status)
{
    ...
    if(friInf.remark.isEmpty())
        ...
        mark = friInf.remark;
	//如果对方是离线或隐身，就不用显示他们的状态
	if(OFFLINE==status || STEALTH==status)
        text = QString("%1(%2)\n").arg(mark).arg(friInf.account);
	else
        text = QString("%1(%2)\n%3").arg(mark)
			.arg(friInf.account).arg(strStatus);
...
void FriendButton::dealMessage(const Message &mes)
{
    ...
    if(friInf.remark.isEmpty())
        ...
        mark = friInf.remark;
    text = QString("%1(%2)\n%3").arg(mark)
        .arg(friInf.account).arg(tr("New Message!"));
    setText(text);
    QPalette   pal;
