...
{
    if(listen(QHostAddress::LocalHost, 666)){
...
void Server::incomingConnection(int sockDescriptor)
{
    ...
    connect(clientSocket, SIGNAL(deleteSignal(const QString &)),this, SLOT(clientDisconnected(const QString &)));
...
void Server::sendMessage(const saveStruct &save)
{
	...
	else if(CHANGE_REMARK == temp.requestKind)
	{
        ...
        clientDisconnected(save.myAccount);
